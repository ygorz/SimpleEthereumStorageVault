...
contract SESVaultTest is Test {
    ...
    function testStoreEthRevertsIfStoringZero() public {
        vm.startPrank(user1);
        vm.expectRevert("Cannot store 0 Ethereum");
        vault.storeEthereum{value: 0}();
        vm.stopPrank();
    }
    ...
    function testWithdrawRevertsIfUserHasNoBalance() public {
        vm.startPrank(user1);
        vm.expectRevert(SimpleEthereumStorageVault.SimpleEthereumStorageVault__UserHasNoBalance.selector);
        vault.withdrawEthereum(USER_ETH_DEPOSIT);
        vm.stopPrank();
    }
    ...
    // Price feed tests
    function testUsdValueOfBalanceRevertsIfBalanceIsZero() public {
        vm.startPrank(user1);
        vm.expectRevert(SimpleEthereumStorageVault.SimpleEthereumStorageVault__UserHasNoBalance.selector);
        vault.getUsdValueOfUserEthBalance();
        vm.stopPrank();
    }

    function testPriceFeedReturnsValue() public depositedEthereum {
        vm.startPrank(user1);
        assertEq(vault.getUsdValueOfUserEthBalance(), 1800e18);
        vm.stopPrank();
    }
}
